...
class CheXpertDataModule(pl.LightningDataModule):
    def __init__(
        ...
        self.test_set_resample = CheXpertDataset(
            self.csv_test_img_resample,
            self.image_size,
            augmentation=False,
            pseudo_rgb=pseudo_rgb,
        )
...
def main(hparams):
    ...
    model = model_type.load_from_checkpoint(
        "/vol/aimspace/users/ngq/Towards-Fair-and-Private-AI-in-Chest-Radiograph-Analysis/disease/chexpert/disease/resnet-all/version_0/checkpoints/epoch=11-step=6108.ckpt",
    ...
    preds_test_resample, targets_test_resample, logits_test_resample = test(
        model, data.test_resample_dataloader(), device
    )
    ...
    embeds_test_resample, targets_test_resample = embeddings(
        model, data.test_resample_dataloader(), device
    )
