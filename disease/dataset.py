...
class CheXpertDataModule(pl.LightningDataModule):
    def __init__(self, csv_train_img, csv_val_img, csv_val_img_resample,csv_val_sex_img_resample,  csv_test_img, csv_test_img_resample, csv_test_sex_img_resample, image_size, pseudo_rgb, batch_size, max_physical_batch_size, num_workers, train_aug):
        ...
        self.csv_val_sex_img_resample= csv_val_sex_img_resample
        self.csv_test_img = csv_test_img
        self.csv_test_img_resample = csv_test_img_resample
        self.csv_test_sex_img_resample = csv_test_sex_img_resample
        self.image_size = image_size
        ...
        self.val_sex_set_resample = CheXpertDataset(self.csv_val_sex_img_resample, self.image_size, augmentation=False, pseudo_rgb=pseudo_rgb)
        ...
        self.test_sex_set_resample = CheXpertDataset(self.csv_test_sex_img_resample, self.image_size, augmentation=False, pseudo_rgb=pseudo_rgb)
        print('#train: ', len(self.train_set))
        print('#val:   ', len(self.val_set))
        print('#val resample:   ', len(self.val_set_resample))
        print('#val resample sex:   ', len(self.val_sex_set_resample))
        print('#test:  ', len(self.test_set))
        print('#test resample:  ', len(self.test_set_resample))
        print('#test resample sex:  ', len(self.test_sex_set_resample))
    ...
    def val_sex_resample_dataloader(self):
        return DataLoader(self.val_sex_set_resample, self.max_physical_batch_size, shuffle=False, num_workers=self.num_workers)
    ...
    def test_sex_resample_dataloader(self):
        return DataLoader(self.test_sex_set_resample, self.max_physical_batch_size, shuffle=False, num_workers=self.num_workers)